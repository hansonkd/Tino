...
async def simple_echo_client_fapi_simple(ntimes):
	async with httpx.AsyncClient() as client:
		t1 = time.time()
		for _ in range(ntimes):
			res = await client.post("http://localhost:9999/fapi/echo/simple", json=simple_string)
			res.json()
		return time.time() - t1
...
async def simple_echo_client_fapi_simple_concurrent(ntimes):
	async with httpx.AsyncClient(timeout=300) as client:
		...
		futures = []
		for _ in range(ntimes):
			futures.append(client.post("http://localhost:9999/fapi/echo/simple", json=simple_string))
...
async def simple_echo_client_fapi_complex(ntimes):
	async with httpx.AsyncClient() as client:
		t1 = time.time()
		for _ in range(ntimes):
			res = await client.post("http://localhost:9999/fapi/echo/complex", json=complex_obj.dict())
